...
namespace LINQ_Console
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //returnMoreColAndUsingAlias();

            //returnColAsArray();

            //returnWithMultiCondetions();
        ...
        private static void resultme()
        {
            // >> Union == Union All

            // >> Get Data:
            var books1 = new BookRepo().GetBooks();
            var books2 = new BookRepo().GetBooks2();

            var returndata = (from a in books1 select a).Union(from b in books2 select b).ToList();
        ...
        private static void returnInnerJoin()
        {
            // >> join == inner join
            ...
            var returndata = from a in books1
                             join b in books2
                             on a.title equals b.title
                             select new { a.title ,a.price };
        ...
        private static void returnGroupsBy()
        {
            // >> Get Data:
            var books = new BookRepo().GetBooks();

            var returndata = from b in books
        ...
        private static void returnWithMultiCondetions()
        {
            // >> Get Data:
            var books = new BookRepo().GetBooks();

            var returndata = from b in books
                             where b.price > 4 && b.title.ToLower().StartsWith("a")
                             select new { BookTitle = b.title, BookPrice = b.price };

            // >> Display:
            foreach (var c in returndata)
            {
                Console.WriteLine(c.BookTitle + " >> " + c.BookPrice);
            }
        }

        private static void returnColAsArray()
        {
            // >> Get Data:
            var books = new BookRepo().GetBooks();

            var moreColWithAlias = from b in books
                                   from r in b.recommendations
                                   select new { BookTitle = b.title, BookPrice = b.price, recommend = r };


            // >> Display:
            foreach (var c in moreColWithAlias)
            {
                Console.WriteLine(c.BookTitle + " >> " + c.BookPrice + " >> " + c.recommend);
            }
        }

        private static void returnMoreColAndUsingAlias()
        {
            // >> Get Data:
            var books = new BookRepo().GetBooks();
